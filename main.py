...
class CustomerTracker:
    ...
    def process_frame(self, frame):
        ...
        zone_coords_2 = get_zone_coords(annotated_frame, self.zone_2) if self.zone_2 else None
        ...
        for track in tracks:
            ...
            if self.zone_2:
                if is_box_in_zone(box, zone_coords_1, 0.01) and not is_box_in_zone(box, zone_coords_2, 0.02):
        ...
        for track in tracks:
            ...
            if track_id in track_customer:
                ...
                if self.save_video:
                    cv2.putText(
                        annotated_frame,
                        f"Track_id: {track_id}, Track_person: {self.track_person[track_id]['name_track_id']} Time_1: {self.track_person[track_id]['total_time']:.1f}s",
                        (box[0], box[1] - 10),
        ...
        for t_id, info in self.track_person.items():
            if self.current_frame - info["last_seen_frame"] > 200:
                if info["total_time"] > 5 and not info["reid"]:
                    send_time_to_kafka(self.camera_id, info["name_track_id"], info["total_time"])
